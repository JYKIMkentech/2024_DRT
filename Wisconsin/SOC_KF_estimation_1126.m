...
Config.cap = 2.7742;     % 명목 용량 [Ah]
Config.coulomb_efficiency = 1; % 쿨롱 효율
noise_vector = 3000; % noise vector 갯수
noise_percent = 0.02;
...
P_init_HPPC_1RC = [1e-9 0;
                   0 1e-3]; % [SOC; V1]

P_init_HPPC_2RC = [1e-9 0    0;
                   0    1e-14 0;
                   0    0    1e-14]; % [SOC; V1; V2]
...
P_init_DRT(1,1) = 1e-10; % SOC에 대한 초기 분산
% V_RC_i에 대한 초기 분산은 모두 0으로 설정

% 프로세스 잡음 공분산 행렬 (DRT 기반)
Q_DRT = zeros(state_dimension);
Q_DRT(1,1) = 1e-20; % SOC에 대한 프로세스 잡음 분산
for i = 2:state_dimension
    Q_DRT(i,i) = 1e-20; % 각 V_RC_i에 대한 프로세스 잡음 분산
end
...
try
    for trip_num = 1:num_trips-15
        ...
        %n = 21;
        %noise_percent = 0.05;
        initial_state = randsample(1:noise_vector, 1);

        % 전류에 노이즈 추가
        [noisy_trip_current, states_trip_current] = add_markov_noise(trip_current, noise_vector, noise_percent, initial_state);
