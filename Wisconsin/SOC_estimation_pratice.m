...
% ECM 파라미터 정렬
soc_sorted = [optimized_params_struct_final_2RC.SOC];
gamma_sorted = gamma_est_all;  % gamma_est_all은 이미 트립별로 정렬되어 있음

%% 2. 공통 설정

Config.cap = 2.7742;     
Config.coulomb_efficiency = 1; % 쿨롱 효율
n = 2000; % noise vector 갯수
noise_percent = 0.02;
...
%% 3. OCV-SOC 도함수 계산 및 이동 평균 적용

% ΔOCV/ΔSOC 계산
delta_soc = diff(unique_soc_values);
delta_ocv = diff(unique_ocv_values);
dOCV_dSOC = delta_ocv ./ delta_soc;

% SOC 중간점 계산 (도함수의 SOC 지점)
mid_soc = (unique_soc_values(1:end-1) + unique_soc_values(2:end)) / 2;

% 이동 평균 윈도우 크기 선택 
window_size = 10;

% 이동 평균 적용 (양쪽에 패딩하여 길이를 맞춤)
dOCV_dSOC_padded = padarray(dOCV_dSOC, floor(window_size/2), 'replicate', 'both');
dOCV_dSOC_smoothed = movmean(dOCV_dSOC_padded, window_size);

% 패딩된 부분 제거하여 원래 길이로 맞춤
dOCV_dSOC_smoothed = dOCV_dSOC_smoothed(floor(window_size/2)+1:end-floor(window_size/2));

% 도함수에 대한 보간 함수 생성
F_dOCV_dSOC = @(soc) interp1(mid_soc, dOCV_dSOC_smoothed, soc, 'linear', 'extrap');

%% 4. ECM 파라미터 준비 (HPPC 기반)
...
%% 5. 칼만 필터 설정 - HPPC (1-RC 및 2-RC 모델)

% 초기 공분산 행렬 (1-RC 및 2-RC 모델)
P_init_HPPC_1RC = [1e-6 0;
                   0 1e-15]; % [SOC; V1]

P_init_HPPC_2RC = [1e-6 0    0;
                   0    1e-15 0;
                   0    0    1e-13]; % [SOC; V1; V2]
...
Q_HPPC_2RC = [1e-17 0        0;
             0     1e-15    0;
             0      0     1e-15];

% 측정 잡음 공분산 (1-RC 및 2-RC 모델)
R_HPPC = 5.25e-1; % 측정 잡음 특성에 따라 조정
...
P_init_DRT(1,1) = 1e-20; % SOC에 대한 초기 분산
for i = 2:state_dimension
    P_init_DRT(i,i) = 1e-5; % 각 V_RC_i에 대한 프로세스 잡음 분산
end

% 프로세스 잡음 공분산 행렬 (DRT 기반)
Q_DRT = zeros(state_dimension);
Q_DRT(1,1) = 1e-6; % SOC에 대한 프로세스 잡음 분산
for i = 2:state_dimension
    Q_DRT(i,i) = 1e-5; % 각 V_RC_i에 대한 프로세스 잡음 분산
end

% 측정 잡음 공분산 (DRT 기반)
R_DRT = 5.25e-1; % 측정 잡음 분산 
...
try
    for trip_num = 1:num_trips-16
        ...
        if trip_num == 1
            ...
            SOC_CC_prev = initial_soc; % 5% 낮게 설정하여 CC SOC 초기값 부정확하게 설정
            ...
            gamma_current_init = gamma_est_all(trip_num, :)'; % 현재 트립의 gamma 값 사용
            delta_theta = theta_discrete(2) - theta_discrete(1);
            R_i_init = gamma_current_init * delta_theta; % num_RC x 1 벡터
            C_i_init = tau_discrete ./ R_i_init;         % num_RC x 1 벡터
            V_RC_init = noisy_trip_current(1) * R_i_init .* (1 - exp(-dt_initial ./ (R_i_init .* C_i_init))); % num_RC x 1 벡터
            X_est_DRT(2:end) = V_RC_init; % num_RC x 1 벡터로 저장
        else
        ...
        for k = 2:num_samples
            ...
            %% CC SOC 계산 (노이즈와 바이어스가 추가된 전류로 쿨롱 카운팅)
            SOC_CC = SOC_save_CC(k-1) + (dt_k / (Config.cap * 3600)) * noisy_ik * Config.coulomb_efficiency; % 쿨롱 효율을 0.98로 설정하여 부정확성 추가
            ...
            % 관측 행렬 H 계산 (ΔOCV/ΔSOC 사용)
            dOCV_dSOC = F_dOCV_dSOC(SOC_pred_HPPC_1RC);
            ...
            % 관측 행렬 H 계산 (ΔOCV/ΔSOC 사용)
            dOCV_dSOC = F_dOCV_dSOC(SOC_pred_HPPC_2RC);
            H_k_HPPC_2RC = [dOCV_dSOC, 1, 1];
            ...
            % 현재 트립의 gamma 값 사용 (보간 없이)
            gamma_current = gamma_est_all(trip_num, :)'; % num_RC x 1 벡터

            % 각 RC 소자에 대한 R_i 및 C_i 계산
            delta_theta = theta_discrete(2) - theta_discrete(1);
            R_i = gamma_current * delta_theta; % num_RC x 1 벡터
            C_i = tau_discrete ./ R_i;         % num_RC x 1 벡터

            % 상태 천이 행렬 A_DRT 계산
            A_DRT = eye(state_dimension);
            A_DRT(2:end, 2:end) = diag(exp(-dt_k ./ tau_discrete));

            % 입력 행렬 B_DRT 계산
            B_DRT = zeros(state_dimension, 1);
            B_DRT(1) = (dt_k / (Config.cap * 3600)) * Config.coulomb_efficiency;
            B_DRT(2:end) = R_i .* (1 - exp(-dt_k ./ tau_discrete));

            % 상태 예측
            X_pred_DRT = A_DRT * X_est_DRT + B_DRT * noisy_ik;
            ...
            Vt_pred_DRT = OCV_pred_DRT + R0_est_all(trip_num) * noisy_ik + sum(X_pred_DRT(2:end));

            % 관측 행렬 H_DRT 계산 (ΔOCV/ΔSOC 사용)
            dOCV_dSOC = F_dOCV_dSOC(SOC_pred_DRT);
...
subplot(4,1,4);
hold on;
plot(all_time_concat, (all_SOC_CC_concat - all_SOC_true_concat) * 100, 'Color', c_mat(2, :), 'LineWidth', 1.5, 'DisplayName', 'CC SOC Error');
plot(all_time_concat, (all_SOC_HPPC_1RC_concat - all_SOC_true_concat) * 100, '--', 'Color', c_mat(3, :), 'LineWidth', 1.5, 'DisplayName', 'N-RC SOC Error (HPPC 1RC)');
plot(all_time_concat, (all_SOC_HPPC_2RC_concat - all_SOC_true_concat) * 100, '--', 'Color', c_mat(4, :), 'LineWidth', 1.5, 'DisplayName', 'N-RC SOC Error (HPPC 2RC)');
plot(all_time_concat, (all_SOC_DRT_concat - all_SOC_true_concat) * 100, '--', 'Color', c_mat(5, :), 'LineWidth', 1.5, 'DisplayName', 'DRT SOC Error');
xlabel('Time [s]', 'FontSize', labelFontSize);
ylabel('SOC Error [%]', 'FontSize', labelFontSize);
title('All Trips: SOC Estimation Error', 'FontSize', titleFontSize);
legend('Location', 'best', 'FontSize', legendFontSize);
grid on;
hold off;
...
%% 10. Markov Chain Noise를 추가하기 위한 함수 정의
