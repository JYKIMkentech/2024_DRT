...
for l_idx = 1:length(lambda_values)
    ...
    for fold = 1:num_folds
        ...
        % 학습 데이터로 gamma 추정
        [gamma_estimated, theta_discrete] = estimate_gamma(lambda, train_scenarios, type_data, tau_min);
        
        % 검증 데이터로 CVE 계산
        cve_total = cve_total + calculate_error(gamma_estimated, val_scenarios, type_data, tau_min, theta_discrete);
    end
...
%% 함수 정의
function [gamma_estimated, theta_discrete] = estimate_gamma(lambda, train_scenarios, type_data, tau_min)
    W_total = []; y_total = [];
    OCV = 0; R0 = 0.1;
    for s = train_scenarios
        ...
        dur = tau_max;
        % DRT_estimation 함수 호출 (lambda는 여기서는 사용되지 않음)
        [~, ~, theta_discrete, tau_discrete, W] = DRT_estimation(t, ik, V_sd, lambda, n, dt, dur, OCV, R0);
...
function error_total = calculate_error(gamma_estimated, val_scenarios, type_data, tau_min, theta_discrete)
    error_total = 0;
    OCV = 0; R0 = 0.1;
    for s = val_scenarios
        idx = find([type_data.SN] == s, 1);
